...
function isCreditCardNumber(ccn) {
  const arrCCN = String(ccn).split('').map((el) => +el).reverse();
  const lastNum = arrCCN[0];
  arrCCN.splice(0, 1);
  let sum = 0;
...
function getDigitalRoot(num) {
  const arr1 = String(num).split('').map((el) => +el);
  const sum1 = arr1.reduce((a, b) => a + b);
  if (sum1 <= 9) {
    return sum1;
  }
  const arr2 = String(sum1).split('').map((el) => +el);
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < firstStr.length; i += 1) {
    if (arr.every((el) => el[i] === firstStr[i])) {
      totalStr += `${firstStr[i]}/`;
    } else {
      break;
    }
