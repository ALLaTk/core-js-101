...
function timeSpanToString(startDate, endDate) {
  ...
  const hour = `${Math.floor((diff / 3600000) % 100)}`.padStart(2, '0');
  const min = `${Math.floor((diff / 60000) % 60)}`.padStart(2, '0');
  const sec = `${Math.floor((diff / 1000) % 60)}`.padStart(2, '0');
  const ms = `${Math.floor(diff % 1000)}`.padStart(3, '0');
...
function angleBetweenClockHands(date) {
  ...
  const hourAngle = (0.5 * min) + (30 * hour);
